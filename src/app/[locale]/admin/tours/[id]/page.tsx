...
const TourDetailPage = () => {
  ...
  const { setCurrentTour, openEditModal } = useTourStore();
  ...
  const handleEdit = () => {
    if (tour) {
      setCurrentTour(tour);
      openEditModal();
      router.push(`/${locale}/admin/tours/edit/${tour.id}`);
    }
  };
  ...
  if (error || !tour) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-4">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Tour không tồn tại</h1>
        <p className="text-gray-600 dark:text-gray-400">Không tìm thấy tour với ID: {tourId}</p>
            ...
            <div className="flex-1 space-y-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{tour.title}</h2>
                  <Badge variant={tour.meta_data.status === "published" ? "default" : "secondary"}>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {tour.meta_data.date_updated && (
                  <div className="flex justify-between">
                    <span>Cập nhật:</span>
                    <span>{new Date(tour.meta_data.date_updated).toLocaleDateString("vi-VN")}</span>
                  </div>
                )}
              </CardContent>
            </Card>
