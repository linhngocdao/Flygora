...
const TourForm = forwardRef<TourFormRef, TourFormProps>(
  ({ onSubmit, onChange, initialData }, ref) => {
    ...
    // Helper function để transform dữ liệu từ API thành form data
    const transformApiDataToFormData = useCallback((apiData: any): Partial<TourFormValues> => {
      if (!apiData) return {};
      ...
      return {
        ...
        card_description: apiData.card_description || "",
        location: apiData.location || "",
        meeting_point: apiData.meeting_point || "",
        age_requirement: apiData.age_requirement || "",
        cover: apiData.cover || "",
        image_header: apiData.image_header || "",
        image_in_menu: apiData.image_in_menu || "",

        // Duration & Pricing
        tour_days: apiData.tour_days || "",
        tour_nights: apiData.tour_nights || "",
        original_price: apiData.original_price ?? 0,
        sale_price: apiData.sale_price ?? 0,
        participant_min: apiData.participant_min ?? 0,
        participant_max: apiData.participant_max ?? 0,

        // Category & Meta
        tour_category_id: apiData.category?.id || "",
        product_code: apiData.product_code || "",
        status: apiData.meta_data?.status || "unpublished",

        // Tour Detail (flat on root)
        tour_booking_process: apiData.tour_booking_process || "",
        tour_cancellation_policy: apiData.tour_cancellation_policy || "",

        // Tour detail (nested)
        campsites: apiData.tour_detail?.campsites || "",
        food: apiData.tour_detail?.food || "",
        kitlist: apiData.tour_detail?.kitlist || "",
        the_area: apiData.tour_detail?.the_area || "",

        // Arrays
        tour_images: apiData.tour_detail_images?.length
          ? apiData.tour_detail_images.map((img: any, i: number) => ({
              image_url: img.image_url || "",
              caption: img.caption || "",
              sort_order: i + 1,
            }))
          : [{ image_url: "", caption: "", sort_order: 1 }],

        tour_intenerary: apiData.tour_intenerary?.length
          ? apiData.tour_intenerary.map((it: any, i: number) => ({
              session: it.session || "",
              title: it.title || "",
              description: it.description || "",
              sort_order: i + 1,
            }))
          : [{ session: "", title: "", description: "", sort_order: 1 }],

        tour_inclusions: apiData.tour_include?.length
          ? apiData.tour_include.map((it: any, i: number) => ({
              title: it.title || "",
              description: it.description || "",
              sort_order: i + 1,
            }))
          : [{ title: "", description: "", sort_order: 1 }],

        videos: apiData.videos?.length
          ? apiData.videos.map((v: any, i: number) => ({
              url: v.url || "",
              title: v.title || "",
        ...
        tour_highlights: apiData.tour_highlights?.length
          ? apiData.tour_highlights.map((h: any, i: number) => ({
              icon: h.icon || "",
              title: h.title || "",
              description: h.description || "",
              sort_order: i + 1,
            }))
          : [{ icon: "", title: "", description: "", sort_order: 1 }],

        // Meta settings
        meta_robot: apiData.meta_data?.meta_robot || [],
        is_featured: apiData.meta_data?.is_featured ?? false,
        is_top: apiData.meta_data?.is_top ?? false,
        show_in_footer: apiData.meta_data?.show_in_footer ?? false,

        // Meta SEO
        meta_title: apiData.meta_data?.meta_title || "",
        meta_description: apiData.meta_data?.meta_description || "",
        meta_keyword: apiData.meta_data?.meta_keyword || "",
        // meta_image: apiData.meta_data?.meta_image || "", // nếu có
      };
    }, []);

    // Effect để set dữ liệu ban đầu khi có initialData
    useEffect(() => {
      if (initialData) {
        const formData = transformApiDataToFormData(initialData);
        form.reset(formData);
      }
    }, [initialData, form, transformApiDataToFormData]);
    ...
    useEffect(() => {
      ...
      const timeoutId = setTimeout(() => {
        if (onChangeRef.current) {
          // Truyền nguyên data để preview, không clean trong lúc edit
          // Chỉ clean khi submit hoặc khi thực sự cần thiết
      ...
      return {
        submitForm: async () => {
          ...
          if (isValid) {
            ...
            // Chỉ clean data khi submit, không clean khi đang edit
        ...
        getFormData: () => {
          // Trả về raw data cho getFormData, không clean
          // Chỉ clean khi submit thực sự
